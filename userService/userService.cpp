...
void UserService::RepositoryType(const std::string &fileType) {
    if(fileType == "csv"){
        ...
        std::string userFile = R"(D:\OOP assignments\a5-6-913-Hornea-Dorian-Alexandru\List.csv)";
        auto* repo = new CSVRepo{userVector, userFile};
        this->userRepository = repo;
    }
    else if(fileType == "html"){
        std::vector<Event> userVector;
        std::string userFile = R"(D:\OOP assignments\a5-6-913-Hornea-Dorian-Alexandru\List.html)";
...
int UserService::filterByMonth(std::vector<Event> &events, std::string &filter_month) {
    ...
    int_month = stoi(filter_month);
    if(filter_month[0] == '\0'){
        std::copy_if(result.begin(), result.end(), std::back_inserter(events), [](Event& event) {return true;});
    }
    else{

        std::copy_if(result.begin(), result.end(), std::back_inserter(events), [&filter_month, &int_month](Event& event){return event.get_month() == int_month;});
    }
    return events.size();
}
